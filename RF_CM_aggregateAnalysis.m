...
alt_thresh = prctile(alt_shuff,90,2); % set really low on purpose so that all recordings are included
azi_thresh = prctile(azi_shuff,90,2);
...
% visualize
scatter(alt_pref(isSpatiallyTuned&isANOVA&isReliable)',CC_mean_coherence(isSpatiallyTuned&isANOVA&isReliable)')
...
mean(CC_mean_coherence(isLeftHalf&isIncluded))
