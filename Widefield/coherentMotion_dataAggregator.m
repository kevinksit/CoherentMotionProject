...
for files = 1:length(matfiles)
    if strcmp(matfiles(files).name,'coherenceMap.mat')
...
coherentMotion_aggregateData(recording_number).coherence_map = coherenceMap;

coherentMotion_aggregateData(recording_number).coherence_curve = coherence_responses;
cd ..
